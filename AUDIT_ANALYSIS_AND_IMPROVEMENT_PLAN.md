...
|---|---|---|
| `pio run -e seeed_xiao_esp32s3` | **Pass** | Firmware compiles and links with the exact pinned dependencies also used by Wokwi. Current static RAM: 49,628/327,680 bytes (15.1%); flash: 956,897/3,342,336 bytes (28.6%). The 16,384-byte UART ring is allocated during GNSS startup and therefore is not included in the static figure. It does not prove runtime behavior. |
...
| `python -m unittest discover -s test -p "test_*.py" -v` | **78/78 pass** | Includes day-mode, track retention/integrity/streaming, Wi-Fi event ownership, configuration/home persistence, active-time accounting, guarded daily-date transitions, and Wokwi circuit/configuration/scenario/analyzer contracts. Date tests cover record validation, torn A/B writes, generation wrap, single forward glitch, confirmed reacquisition, backward almanac date, calendar-boundary midnight, data loss/untrusted boundaries, journal failure/retry, and source integration. These are host-side, not HIL. |
| Wokwi CLI functional, mode, fault and high-rate scenarios | **Pass; final fault rerun quota-blocked** | Executes the real ESP32-S3 firmware and an adjustable 1–5 Hz NMEA chip. Boot, four runtime modes, day mode, geofence, persistence, eleven GNSS profiles and recovery have passed. The new 5 Hz capture processed 239 firmware-visible NMEA sentences with zero UART overflow; independent VCD decoding found 244 valid NMEA frames and a 5 Hz maximum. A 743.9 m jump was rejected and retained in diagnostics. A repeat of the fault VCD was blocked only after the account exhausted its free monthly CI minutes. Wokwi does not model electrical power, RF, antenna, or SK6812 RGBW white-channel behavior. |
