...
|---|---|---|
| `pio run -e seeed_xiao_esp32s3` | **Pass** | Firmware compiles and links with the exact pinned dependencies also used by Wokwi. Current static RAM: 54,084/327,680 bytes (16.5%); flash: 965,637/3,342,336 bytes (28.9%). The 16,384-byte UART ring is allocated during GNSS startup and therefore is not included in the static figure. It does not prove runtime behavior. |
...
| `python -m unittest discover -s test -p "test_*.py" -v` | **100/100 pass** | Includes day-mode, track retention/integrity/streaming, Wi-Fi event ownership, configuration/home/live-metrics/session persistence, active-time accounting, guarded daily-date transitions, `millis()` rollover, and Wokwi circuit/configuration/scenario/analyzer contracts. Session tests model strict record validation, torn inactive writes, generation wrap, before/after-commit power cuts, exact-once boot recovery, and the three-entry ring. These are host-side fault models, not physical HIL. |
| Wokwi CLI functional, mode, fault, rate, latency and session-reboot scenarios | **Pass** | Executes the real ESP32-S3 firmware and an adjustable 1–5 Hz NMEA chip. Boot, four runtime modes, day mode, geofence, persistence, eleven GNSS profiles and recovery have passed. The dedicated session scenario obtained trusted GNSS data, performed two software reboots, and observed history advance exactly 0→1→2 with zero save failures; its 15.96 s capture decoded 30 valid NMEA frames at a stable 1 Hz with zero UART overflow. Wokwi does not model electrical power, RF, antenna, flash-write power interruption, or SK6812 RGBW white-channel behavior. |
...
### AUD-019 — Resolved Medium — Long Wokwi captures lost GNSS evidence and transient segment rejections

**Evidence:** the original analyzer recorded both continuously animated 24-pixel buses together with GNSS. Its default one-million-transition buffer stopped near 20 simulated seconds, before the later fault profiles. Raising it to three million made a roughly 14-second scenario exceed five minutes of wall time. The CLI 0.26.1 exported only the first of two analyzer parts. Separately, `last_segment_reject_reason()` was overwritten by the next 5 Hz sample before the two-second diagnostic log could expose a position jump.

**Impact:** a scenario could pass on serial state while its electrical capture silently omitted the fault under investigation; short GPS jumps could also disappear from both field diagnostics and deterministic automation.

**Implemented:** the helper now generates `full` and `gnss` instrumentation views from the one canonical diagram. The short boot test captures WS2812, status and GNSS; long scenarios disconnect the virtual strip inputs and record only UART/tick with a right-sized buffer. Scenarios can disable only LED transport while preserving effect calculations and decision logs. The analyzer decodes 9600-8N1 from scoped VCD channels, validates NMEA checksums and applies profile-specific signal requirements. Small/large segment rejection counters are now monotonic and logged. The verified 5 Hz run captured its full 31.0 seconds, decoded 17,595 UART bytes and retained the large-jump event.

### AUD-020 — Resolved Medium — Rejected speed became diagnostically indistinguishable from valid zero speed

**Evidence:** the speed-spike GNSS profile emits a trusted-position RMC at 80 km/h. `gps.cpp` correctly rejected it, stored a safe filtered speed of `0.0`, reset the distance baseline and did not accumulate activity. `main.cpp`, however, reconstructed `usable` from the already-filtered value (`0 <= SPEED_MAX_VALID_KPH`), producing `speed_kph=0.00 usable=1` and making a rejected spike look identical to a valid stationary observation.

**Impact:** collar effects and metrics remained safe, but serial logs, portal telemetry and automated investigations could draw the wrong conclusion about receiver quality. That ambiguity is especially costly during field diagnosis because the raw invalid value is deliberately discarded.

**Implemented and verified:** GPS state now preserves an explicit `speed_usable` bit alongside the filtered speed, clears it on stale input, and exposes it through the public GPS interface, `/api/status`, `/api/dev`, `[MOTION]` logs and the Wokwi analyzer. The full fault matrix now asserts the invalid state. A quota-efficient dedicated scenario injects 80 km/h and then normal 12 km/h input. It passed with two `usable=0` reports, no added distance/activity during rejection, safe range 1, recovery to `usable=1`/range 7, zero UART overflow or fatal markers, and an independent 17.0 s VCD containing 32 checksum-valid NMEA frames at exactly 1 Hz.

### AUD-021 — Resolved Medium — Periodic diagnostics hid blocking Wi-Fi queries

**Evidence:** the original `[SYS] loop_max_us` combined application work with the UART time needed to print the diagnostic itself, initially reporting approximately 225 ms without attribution. Phase instrumentation isolated a 145,323 us radio maximum. Internal Wi-Fi timing then identified `WiFi.softAPgetStationNum()` at 145,288 us in Wokwi and, after removing that frequent call, exposed a separate periodic `WiFi.channel()` query at 107,664 us. Neither query represented GNSS, LED rendering, HTTP or storage work.

**Impact:** the one-second AP station poll caused avoidable periodic loop stalls in the simulated firmware and could make effects appear to hitch. It also obscured the actual health of GPS processing. Hardware timing may differ, but the calls were redundant in the code: AP connect/disconnect events already update client state, overflow forces reconciliation, AP start records the configured channel, and `STA_GOT_IP` records a channel that can actually have changed.

**Implemented and verified:** `[SYS]` now separates total loop latency, application work and logger emission, then reports maxima for GPS, simulation control, geofence, storage, radio, LED and HTTP. `[WIFI_DIAG]` and `/api/dev` expose AP-count and channel-query maxima; the analyzer preserves both groups. Simulation mode/day commands now avoid NVS writes when their requested state is unchanged. AP client accounting remains immediate through the bounded event queue, retains same-tick reconciliation after queue overflow and uses a 60-second safety poll instead of one second. The redundant periodic channel query was removed while the `STA_GOT_IP` update remains. In the identical 13.0 s scenario, radio maximum fell from 145,323 us to 50 us and loop work maximum from 157,963 us to 79,867 us. The run retained trusted motion, zero UART overflow, 24 checksum-valid NMEA frames, zero invalid frames and a stable 1 Hz GNSS tick. A physical timing run with clients connecting/disconnecting remains desirable because Wokwi latency is not an ESP32 RF benchmark.

### AUD-022 — Resolved Medium — Diagnostics and Wi-Fi getters could still stall the main loop

**Evidence:** after AUD-021, the same scenario still measured a 165,778 us logger maximum and a 224,834 us total loop maximum. Raising only the Wokwi console baud to 460,800 reduced total latency but not enough. A 4 KiB asynchronous driver TX ring was explicitly rejected: it moved the wait into the following HTTP phase (151,769 us) and raised application work to 226,756 us. A fixed application queue removed that transfer, after which per-category timing isolated the remaining delay: the Wi-Fi log slot took 95,754 us while GPS link, GPS fix, motion and LED slots each took under 1.9 ms. Caching `softAPIP()` alone did not change it; replacing the periodic `WiFi.getMode()` driver query reduced the Wi-Fi slot to 943 us.

**Impact:** rich diagnostics could pause GNSS servicing, animations and the captive portal even when no user or radio activity occurred. The same synchronous mode/IP getters also existed in portal and LED status paths. On a battery DIY collar this is mainly a responsiveness, measurement-integrity and debugging problem—not a credential/security issue.

**Implemented and verified:** periodic diagnostics now enter a fixed 4 KiB no-heap queue and drain at most 64 bytes per loop only when the active serial sink reports capacity. The queue exposes pending/dropped-byte counters; the passing run reported zero drops and an 83 us drain maximum. Five rotating detail slots preserve the previous per-category cadence while avoiding one monolithic report, and system/Wi-Fi diagnostics use separate slots. `wifi_mgr` now owns and caches every successful Wi-Fi mode transition and queries the configured AP IP only once when the AP starts; logger, portal and LED consumers use those cached values. The analyzer enforces 60 ms emit, 5 ms drain, 120 ms application-work and existing radio/query limits. The final Wokwi run passed with 1,894 us logger emission, 77,712 us total loop, 76,519 us application work, 2,059 us HTTP, 50 us radio, 120 us AP reconciliation, zero channel-query time, zero UART overflow, zero invalid NMEA frames and a stable 1 Hz GNSS tick. All 82 host tests pass. Both targets compile; the physical XIAO image uses 53,844 B RAM (16.4%) and 960,469 B flash (28.7%). Physical serial is USB CDC while Wokwi is a hardware UART, so the bounded drain is generic over both sink types and real-device timing remains a useful follow-up measurement.

### AUD-023 — Resolved Medium — Live daily metrics could be torn across independent NVS keys

**Original evidence:** `save_metrics()` wrote date, distance, active time, maximum speed, and update minute as five independent Preferences values and ignored every return value. `load_metrics()` validated individual ranges but had no generation or common integrity field, so a power loss could leave a range-valid mixture such as a new date with the previous day's distance. A second cut after the completed-day journal but before resetting the live keys could also expose an already completed day again after reboot.

**Impact:** battery removal, a loose connector, brownout, or reset during the save window could silently corrupt the collar's daily distance/activity summary even though every individual value looked valid. This is data integrity for the actual DIY use, not a security concern.

**Implemented and verified:** all live daily fields now form one fixed 36-byte `MetricsRecord` with magic, format version/size, flags, reserved format space, wrap-safe generation, strict date/value validation, and CRC32. Saves write only `met_a` or `met_b`, require the exact byte count, decode the inactive record, and require byte-for-byte readback before selecting it. A missing/corrupt mirror is repaired on the next save even if the payload did not change; unchanged healthy state causes no extra flash write. Legacy independent keys migrate once into both records and cannot be resurrected after the migration marker exists. Boot selects the newest valid generation and uses the completed-day journal to discard a stale live snapshot from the narrow journal/reset power-loss window. `/api/dev` exposes slot, generation, failures, and recovery count. Seven new host tests cover strict format validation, empty pre-fix state, torn/corrupt writes, cross-field coherence, generation rollover, completed-day reconciliation, migration, and firmware wiring; all 93 tests pass. The Wokwi modes/persistence scenario passed a real ESP32-S3 software reboot while retaining configuration/mode behavior, trusted GPS, four runtime modes, day mode and geofence, with 72 valid NMEA frames and zero overflow. Both targets compile: Wokwi uses 53,884 B RAM/962,089 B flash and physical XIAO uses 53,908 B RAM/963,121 B flash. Session/history was deliberately kept out of this change and is now resolved independently in AUD-024.

### AUD-024 — Resolved High — Session recovery and history updates were not one transaction

**Original evidence:** the current session payload (`s_cur`), open marker (`s_open`), three history entries (`h0`–`h2`), count, ring index, and version were written as separate Preferences keys without checking the returned byte counts. On boot, recovery first appended the previous open session and then cleared its marker in another write. A power interruption between those operations could append the same session again on the next boot, lose the new history entry, expose mismatched ring metadata, or clear otherwise usable history.

**Real-life impact:** a loose battery connector, brownout from LEDs/radio, reset, or intentional power cycle could damage the collar's last-three-walk history precisely when that history is being recovered. The owner could see duplicates, missing trips, or an empty history even though the GPS and collar otherwise worked normally.

**Implemented and verified:** history, ring metadata, the current session, and its open state now form one fixed 132-byte `SessionStoreRecord`. It has magic, format version/size, reserved bytes, wrap-safe generation and CRC32. Semantic validation rejects unknown flag combinations, impossible/unsorted dates, nonzero payload in an empty/no-fix session, inconsistent average speed, malformed ring shape, and invalid unused slots. Saves write the complete candidate only to inactive `ses_a`/`ses_b`, require the exact byte count, decode the readback, and require byte-for-byte equality before selecting it. A missing mirror is repaired; unchanged healthy state does not consume another flash write. Legacy split keys migrate once and cannot later resurrect stale data. Boot finalization of the previous session and opening of the new one are committed together, so a cut selects either the old complete open session or the new complete `{history,current}` generation—never a half-applied append. `/api/dev` and Wokwi status expose slot, generation, save failures, recovery count, and history count.

Seven host tests model torn writes, corruption, generation rollover, cuts immediately before and after commit, exact-once recovery, and five sessions wrapping the three-entry ring. The dedicated Wokwi test obtained a trusted fix and verified history `0→1→2` across two ESP32-S3 software reboots with zero storage failures, 30/30 valid decoded NMEA frames, stable 1 Hz GNSS, and zero UART overflow. All 100 host tests pass. The physical target compiles at 54,084 B RAM (16.5%) and 965,637 B flash (28.9%). Cutting real power at individual NVS write boundaries remains the appropriate final HIL test.
...
1. **Completed:** runtime configuration, home, live daily metrics, completed-day journal, and session/history use scoped validated, read-back-verified A/B records with wrap-safe generations.
...
For the stated DIY objective, the informational observations require no code change. Track retention/integrity/streaming, Wi-Fi event ownership, configuration/home/live-metrics/session recovery, active-time accounting, guarded daily-date rollover, and `millis()` rollover handling are corrected and host-tested. The next strongest evidence is physical power-cut fault injection at NVS write boundaries, followed by recorded-NMEA and throttled-export testing; the next bounded tooling fix is the cross-platform portal test runner in AUD-014.
