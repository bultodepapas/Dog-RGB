...
|---|---|---|
| `pio run -e seeed_xiao_esp32s3` | **Pass** | Firmware compiles and links with the exact pinned dependencies also used by Wokwi. Current static RAM: 49,676/327,680 bytes (15.2%); flash: 958,009/3,342,336 bytes (28.7%). The 16,384-byte UART ring is allocated during GNSS startup and therefore is not included in the static figure. It does not prove runtime behavior. |
...
| `python -m unittest discover -s test -p "test_*.py" -v` | **93/93 pass** | Includes day-mode, track retention/integrity/streaming, Wi-Fi event ownership, configuration/home/live-metrics persistence, active-time accounting, guarded daily-date transitions, `millis()` rollover, and Wokwi circuit/configuration/scenario/analyzer contracts. Persistence tests cover strict record validation, torn A/B writes, generation wrap, one-time migration, completed-day recovery, configuration reset, and source integration. Wokwi contracts require speed validity to survive filtering, idempotent simulation controls, phase-attributed loop diagnostics, enforced latency limits and event-driven AP client accounting with fallback reconciliation. These are host-side, not HIL. |
| Wokwi CLI functional, mode, fault, rate and latency scenarios | **Pass** | Executes the real ESP32-S3 firmware and an adjustable 1–5 Hz NMEA chip. Boot, four runtime modes, day mode, geofence, persistence, eleven GNSS profiles and recovery have passed. The 43.0 s fault capture independently found 54 valid and 8 intentionally invalid NMEA frames with zero UART overflow. The 5 Hz capture processed 239 firmware-visible NMEA sentences; VCD decoding found 244 valid frames and a 5 Hz maximum. A 743.9 m jump was rejected and retained in diagnostics. A focused 17.0 s regression proved an 80 km/h spike remains unusable and recovers correctly. The latency scenario reduced the measured radio maximum from 145,323 us to 50 us and loop work from 157,963 us to 79,867 us while retaining zero overflow and 24/24 valid NMEA frames. Wokwi does not model electrical power, RF, antenna, or SK6812 RGBW white-channel behavior. |
...
1. **Runtime configuration, home, and live daily metrics completed:** use independent validated, read-back-verified A/B blobs with wrap-safe generations. Session/history remains a separate scoped persistence fix.
...
For the stated DIY objective, the informational observations require no code change. Track retention/integrity/streaming, Wi-Fi event ownership, configuration/home/live-metrics recovery, active-time accounting, guarded daily-date rollover, and `millis()` rollover handling are corrected and host-tested. The next valuable code correction is transactional session/history persistence; the next hardware evidence is real power-cut, recorded-NMEA, and throttled-export testing.
