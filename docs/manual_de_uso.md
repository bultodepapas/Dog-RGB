...
## Variante Display experimental — actualización I6d, 2026-09-12

Actividad muestra distancia registrada/fecha, velocidad válida, GPS y modo de
luces; Wi-Fi muestra AP/STA, nombres e IP actuales. Estado muestra recepción GPS
y política efectiva de luces: Modo día, avisos y salida pausada. No verifica
la respuesta física de las tiras. BOOT recorre Actividad → Wi-Fi → Estado → Actividad. Si está oscura, la primera
...
[guía actual](../Platformio/Dog-RGB/docs/display-i6d.md) y
...
Las escenas integradas son **Alta visibilidad** (Chase + Safety Amber), **Calmado** (Breath + Night Red), **Activo** (Comet + Forest) y **Fiesta** (Rainbow + Pride). En `/config`, **Escenas y paletas** permite aplicarlas, copiarlas a uno de los cuatro slots de usuario, editar color/efecto/paleta/transición/mirror, decidir si participan en Show, guardar, borrar, exportar e importar. La vista del collar es aproximada y no sustituye la comprobación sobre las tiras RGBW reales. Aplicar manualmente es volátil: no cambia el modo ni escribe flash, y termina al cancelar, cambiar el modo o reiniciar. La [API local](api-reference.md#led-scenes-api-v1) ofrece las mismas operaciones para clientes personalizados.
