...
The Phase 1 database, four Edge gateways, shared contracts, and deterministic simulator now pass the clean local gate. The migrated one-million-point capacity gate also passes after replacing a per-row telemetry RLS membership plan with one hashed visible-collar subplan; the same storage and query-shape evidence passed on a clean Ubuntu CI runner. The local [deletion-cascade drill](cloud/phase1-deletion-drill.md) inventories all 17 dog-dependent tables and proves complete deletion plus cross-dog/account isolation. The [isolated restore drill](cloud/phase1-restore-drill.md) now verifies exact application hashes, Auth linkage, functions, effective privileges/RLS and owner/non-member behavior without persisting the backup. Disposable hosted concurrency/network measurement, export/purge/tombstone prerequisites, a managed hosted restore, and the remaining operational drills stay open.
