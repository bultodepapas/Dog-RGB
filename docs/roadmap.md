...
- Four LED modes, 12 effects, status pixels, welcome animation, and optional Day Mode.
...
- Evaluate a calibrated per-frame current estimator/limiter only after measuring the real strips and converter.
...
These ideas are explored in the [WLED lessons and implementation plan](analisis-wled-y-plan-implementacion.md). They are optional design directions, not delivered features.
