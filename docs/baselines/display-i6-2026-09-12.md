...
## Owner-requested pause and documentation reconciliation

After I6a the owner requested a development pause and detailed remaining-work
planning. Firmware and the observed image were retained; no new flash or board
test belongs to this documentation revision. Physical BOOT/legibility and
joint acceptance remain open, not implicitly accepted by the pause.

The [incremental contract](../PLANS/2026-09-12_display-incremental-delivery.md)
now maps V1 to final optical/button/radio observation, V2 to staged physical
I0–I3 and V3 to I4 joint acceptance with the final UI. I6b transition, I6c timeout,
I6d State and I6e estimated pause are separate proposed packages with entry,
work and exit criteria. I7 decisions remain optional. Usage, AI workflow,
supporting research, roadmap, queue and entry-point guides were reconciled;
historical measurements above were preserved.

Documentation verification covered local Markdown targets, balanced fenced
blocks and `git diff --check`. The nine PNGs and source hashes still match the
I6a manifest, and the preserved flashed image retains its recorded SHA-256.
No firmware suite was rerun for these documentation-only changes; the build,
host and bench results above belong to the implementation increment.

## Subsequent owner acceptance and resumption

The owner later confirmed improved appearance and actual BOOT page changes.
A separate manually darkened observation confirmed wake-only first press and
navigation on the second. These specific I6a observations are now accepted;
complete optical/radio and joint-load cases remain open. The later
[I6c baseline](display-i6c-2026-09-12.md) records the renewed development and
opt-in inactivity experiment; it does not alter the historical results above.
