...
**Still open:** a deliberately disposable hosted capacity/concurrency/network run; owner-authorized export/purge/job/tombstone prerequisites; a managed backup/PITR restore into a distinct hosted project; and the remaining outage/rotation/quota operational drills. Phase 1 is not complete while these items remain.
