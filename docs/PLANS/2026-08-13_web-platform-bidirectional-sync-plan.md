...
**Status:** Active implementation contract; M0 and M1.1–M1.10 are complete on reviewed local and CI evidence, M1.11–M1.15 are complete on reviewed local evidence, and M1.16 is next. Remote CI was intentionally not run or inspected for M1.11–M1.15.

**Last senior review:** 2026-08-26 (America/Bogota).

**Reviewed repository baseline:** `c28232a8ed8b63f10548301d141d07a7ffc17287` (`main`; the reviewed M1.14 baseline before this M1.15 implementation). The M1.15 local implementation commit is recorded in Git history. The last inspected remote evidence remains M1.10 GitHub CI run [`32911228533`](https://github.com/bultodepapas/Dog-RGB/actions/runs/32911228533); no M1.11–M1.15 CI claim is made.

**Current milestone:** M1D — Local end-to-end gate; M1.1–M1.15 are complete and M1.16 is next.

**Next executable task:** complete only M1.16: enumerate every current private browser/server/Edge/database artifact surface, seed bounded synthetic canaries, and prove that unauthenticated/static/cached responses, action payloads, console/log/error output, URLs, browser storage, retained test artifacts, and external requests contain no prohibited secret, identity, internal-error, or route payload. Reuse the M1.13–M1.15 runner and exact allowlists; do not absorb M1.17 accessibility, M1.18 performance, hosted deployment, firmware work, or a generic security platform.

**Current blocker:** no external blocker. Before M1.16 implementation, freeze the exact prohibited-value classes, observable surfaces, synthetic canaries, and allowed private owner payloads; identify which logs/artifacts are actually produced by the current local stack. Do not add third-party scanning, hosted traffic, broad penetration testing, product features, or assertions that a clean source scan proves runtime privacy. Verification must continue to use the checksum-verified isolated Node `24.18.0` runtime required by M0.1.
...
- [x] ✅ Local Supabase migration stack exists with explicit schemas/grants/RLS, ownership, claims, credentials, sync receipts, raw telemetry, configuration LWW, limits, deletion jobs, retention, tombstone replay, the measured History ordering index, serialized web configuration mutation semantics, accepted capability persistence, bounded pre-ACK queue snapshots, serialized sync/revoke semantics, and live-Auth guards on retained user RPC results.
  - Evidence: 16 migrations and 21 database pgTAP files, introduced across commits `4698f24` through the reviewed local M1.15 baseline; M1.15 required no schema change.
...
- [x] ✅ The Next.js workspace and visual shell are scaffolded with pinned dependencies.
  ...
  - Boundary: the current app completes M1.1–M1.15: Supabase/Auth boundaries, fresh server authorization, protected shell, transactional dog creation, ephemeral claim issuance, simulator pairing/upload, Today, keyset-paginated History, recording detail/points, brightness desired/reported convergence, bounded collar diagnostics/revoke, the twice-clean automated owner journey, the twice-clean identity/object-authorization gate, and the twice-clean deterministic transport/fault gate. M1.16–M1.18 cross-cutting gates remain pending.
...
- [x] ✅ Product web application vertical features: recording detail, brightness desired/reported, and collar diagnostics/revoke.
  ...
  - Boundary: the unified owner journey, identity/object-denial gate, and local deterministic transport/fault gate are complete; M1.16–M1.18 privacy/accessibility/performance gates remain pending and must not be inferred from earlier feature or fault evidence.
...
The text above records the original M0 recovery sequence. Its former evidence gaps were subsequently closed. The broad M1.14 local review, on 2026-08-26 with isolated Node `24.18.0`, replayed all 16 migrations from a clean reset, passed 492/492 pgTAP assertions, database lint/advisors, generated-asset/contracts/lint/type/unit/secret checks, 122/122 portal tests, and the production portal build. The M1.15 review rebuilt the production portal, then the combined M1.13/M1.14/M1.15 runner passed the owner journey twice, the exact two-owner/editor/viewer/deleted-Auth authorization matrix twice, and all ten deterministic fault outcomes twice. Each phase had its own migration/seed reset with zero retry; each fault cycle also proved one preserved full-stack restart and both forced sync/revoke lock orders, followed by reusable teardown. M0 and M1.1–M1.15 are complete on the evidence scopes stated in their ledger entries; remote CI was not run or inspected for M1.11–M1.15.
...
| M1 | Simulator-driven local web vertical slice | **In progress — M1.16 next** | firmware Internet integration |
...
Research was refreshed on 2026-08-24 using primary/official sources and rechecked on 2026-08-26 through the M1.15 boundary. The implementation consequence is part of the plan; links must be revalidated when their phase begins.
