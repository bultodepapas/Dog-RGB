...
**Status:** Active implementation contract; M0 and M1.1–M1.9 are complete on reviewed local and CI evidence, and M1.10 is next.

**Last senior review:** 2026-08-25 (America/Bogota).

**Reviewed repository commit:** `c43313747d154f91b49f888f772b9f924f6e6e2c` (`main`; GitHub CI run [`32904026991`](https://github.com/bultodepapas/Dog-RGB/actions/runs/32904026991) passed every required job).

**Current milestone:** M1C — Minimal useful portal; M1.1–M1.9 are complete and M1.10 is next.

**Next executable task:** complete only M1.10 by first freezing the missing plain-preview time-gap threshold, then replacing the existing protected `/app/[dogId]/recordings/[recordingId]` placeholder and enabling exactly one non-prefetched History detail link per row. Show exact recording/collar metadata, a 100-row keyset-paginated point table, and a provider-neutral preview of only the current point page; break geometry on invalid fixes, explicit `GAP`, sequence discontinuities, the frozen time threshold, and page boundaries. Reuse the authorized RLS DAL and existing schema. Do not add summaries or inferred activity/duration, an external map/tile provider, a map library, client fetching, Realtime, polling, writes, schema/RPC changes, simulator sync, firmware work, or M1.11+ behavior.

**Current blocker:** no external blocker. M1.10 line geometry has one explicit acceptance prerequisite: fill and approve its numeric time-gap threshold before implementing the preview; metadata/table work may proceed without it. The workstation's global Node remains `24.12.0`; verification must continue to use the checksum-verified isolated Node `24.18.0` runtime required by M0.1.
...
- [x] ✅ Local Supabase migration stack exists with explicit schemas/grants/RLS, ownership, claims, credentials, sync receipts, raw telemetry, configuration LWW, limits, deletion jobs, retention, tombstone replay, and the measured History ordering index.
  - Evidence: 13 migrations and 17 database pgTAP files, introduced across commits `4698f24` through `c43313747d154f91b49f888f772b9f924f6e6e2c`.
...
| M1 | Simulator-driven local web vertical slice | **In progress — M1.10 next** | firmware Internet integration |
...
- [ ] M1.10 Recording detail shows metadata plus an accessible point/segment table and provider-neutral plain route preview.
  ...
  - Recording boundary: expose only exact stored metadata needed by the page: collar name fallback, start/end times, timezone at start, state, boot sequence, first/last point sequence, point count, clock quality, telemetry schema, and firmware version. Validate identity, ranges, timestamps, and nullable pairs; render recording and point times in the stored `timezone_at_start`, not the dog's current timezone. Show unavailable/incomplete truth explicitly and never invent duration, distance, activity, coverage, or a `walk` label. Return deeply frozen DTOs and keep the dynamic response `private, no-store`.
  - Point/page boundary: query only rows matching the exact recording `collar_id` and `boot_sequence`, bounded inclusively by its validated first/last point sequences. Select only `point_sequence`, `recorded_at`, `lat_e7`, `lon_e7`, `reported_speed_cmps`, `satellites`, `flags`, and `time_quality`; order by `point_sequence ASC`. Page size is 100 and the query fetches at most 101 rows. Accept at most one nonrepeated `after` matching canonical decimal `0|[1-9][0-9]{0,9}`, require exact string round-trip and unsigned 32-bit range, and—after validating the recording—require `first_point_sequence <= after < last_point_sequence`; reject any `after` when bounds are null. Malformed/out-of-range input gets a bounded first-detail-page recovery and no point query. The 101st validated row proves another page, and the next link uses the 100th row's sequence. No offset, total count, arbitrary order, backward pagination, or unbounded point load.
  - Preview/gap boundary: render a dependency-free, server-generated SVG preview of only the current point page, labelled as a plain orientation aid rather than a map. Source coordinates stay inside the private `no-store` response and are never sent to a third party. A valid drawable point requires `FIX_VALID (flags & 0x01)`, valid E7 coordinates, and no explicit `GAP (flags & 0x20)`. Start a new numbered segment after every invalid fix, explicit gap, nonconsecutive point sequence, interval above the approved time-gap threshold, or page boundary; never connect across one. A single/zero-span position renders a centered marker, while zero drawable positions renders an explicit no-preview state. The accessible table—with `<caption>`, column headers, exact `<time>` values, segment/gap labels, coordinates, metric speed, satellites, human-readable flag meanings, and time-quality labels/unavailable copy—is authoritative; raw numeric flags alone are insufficient. At narrow widths, contain the full table in a labelled keyboard-scrollable region without causing page-level overflow or dropping columns. Make no tile, geocoding, analytics, or other third-party route request.
  - Gap-threshold prerequisite: the frozen Track v3 contract requires time intervals beyond the analytics gap threshold to break geometry, but no numeric threshold is currently recorded. Before any SVG line is implemented, fill `plain-preview time-gap threshold: ______ seconds`; `decision owner: ____________________`; `fixture/rationale evidence: ____________________`. It must accommodate the frozen moving/stationary cadence plus measured jitter while breaking missing coverage. Until approved, implement only metadata/table states or render points without connecting lines; do not borrow the unrelated current-firmware three-second activity threshold.
  - State/test boundary: cover invalid route IDs, inaccessible/cross-dog IDs, revoked-collar history, malformed recording bounds, zero/fewer/exactly/more than 100 points, single/zero-span/all-null positions, invalid-fix and `GAP` flags, sequence/time gaps, equal/null/regressing/future/malformed times, repeated/noncanonical/out-of-range `after`, an inserted later sequence between page requests, point/read failure, and a preview that cannot be drawn. Validate the stored-time/time-quality/`TIME_TRUSTED` relationship without converting unknown time to ingestion time. Prove stable order, bounded query count/limit, no duplicates/skips in the fixed fixture, no false segment connection, exact table semantics, keyboard navigation, reduced-motion compatibility, contained table scrolling with no page overflow at 320/428/768/1280 px, and zero external route/network requests.
  ...
  - Target date/window: ____________________
  - Implementation commit/PR: ____________________
  - Evidence artifact or command: focused recording/point DTO, authorization, pagination, and segmentation tests; pgTAP and raw REST owner/viewer/non-member/anonymous matrix; representative query plans; browser empty/one-page/multi-page/gap/malformed states; no-external-request proof; `npm run phase1:check`; clean local gate; production build; Next.js runtime/cache checks; keyboard/responsive/reduced-motion/accessibility review; CI: ____________________
  - Decision/result: ____________________
