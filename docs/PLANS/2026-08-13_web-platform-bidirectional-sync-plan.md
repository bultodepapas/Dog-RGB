...
The local [Phase 1 deletion drill](../cloud/phase1-deletion-drill.md) adds 13 transactional assertions over two independently owned dogs. It locks the recursive cascade inventory to all 17 current dependent tables, audits every project foreign key for an effective leading index, proves dog deletion removes raw/derived/receipt/credential/loss/configuration state without touching the control dog, and verifies the required account ordering plus profile/membership cleanup and retained-audit anonymization. This closes the active-database cascade primitive only; owner authorization, export, jobs/receipts, tombstone replay, and managed hosted restore remain deliberately unimplemented gates.

The clean local command now also performs a complete logical backup and restores it into a randomly named separate database in the disposable local Supabase cluster. A coordinate-free manifest must match every `api`/`private` table count and full-content hash, Auth linkage, route and config-head hashes, migrations/extensions, project functions, RLS policies and effective privileges; authenticated owner/non-member probes must also remain identical. The backup is held only in memory, the isolated database is removed in a `finally` path, and CI retains only the manifest. Detailed scope and the managed-hosted boundary are recorded in [the Phase 1 restore report](../cloud/phase1-restore-drill.md). This closes local logical recoverability drift, not the required restore of a managed backup/PITR point into a distinct hosted project or deletion-tombstone replay.

Migration `20260818014827_phase1_deletion_jobs.sql` now adds the local owner-authorized dog-deletion primitive. A narrow authenticated RPC serializes exact request replay, verifies current ownership, revokes device ingress, captures a coordinate-free 18-class count manifest, writes an append-only tombstone/job, marks the dog deleting, and removes memberships in one transaction. RLS and membership-backed RPCs therefore lose access immediately. A private `service_role` worker claims one ready job with `SKIP LOCKED`, deletes at most 10,000 points per transaction, checks cumulative counts, cascades the remaining graph only after raw points are gone, and emits a hashed receipt. Thirty-four pgTAP assertions cover privilege boundaries, replay, cross-user denial, exact batches, forced failure rollback/retry and cross-dog survival. The clean HTTP drill sends two simultaneous owner RPCs, proves one job/tombstone, observes outsider `403`, deletes six simulator points in three two-row commits, and retains only coordinate-free CI evidence. Cron is not scheduled and no user-facing workflow is enabled; export, reauthentication UX, account deletion, retention purge and tombstone replay remain separate gates. Detailed scope is in [the Phase 1 deletion report](../cloud/phase1-deletion-drill.md).
...
**Still open:** a deliberately disposable hosted capacity/concurrency/network run; pre-deletion export and strong-confirmation UI, account deletion, scheduled retention purges, and tombstone export/replay; a managed backup/PITR restore into a distinct hosted project; and the remaining outage/rotation/quota operational drills. Phase 1 is not complete while these items remain.
