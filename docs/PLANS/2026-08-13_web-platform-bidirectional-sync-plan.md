...
**Status:** Active implementation contract; M0 and M1.1–M1.10 are complete on reviewed local and CI evidence, M1.11–M1.16 are complete on reviewed local evidence, and M1.17 is next. Remote CI was intentionally not run or inspected for M1.11–M1.16.

**Last senior review:** 2026-08-26 (America/Bogota).

**Reviewed repository baseline:** `5dd2e7206dd1d6442b99874195f44c7bfbf45601` (`main`; the reviewed M1.15 baseline before this M1.16 implementation). The M1.16 local implementation commit is recorded in Git history. The last inspected remote evidence remains M1.10 GitHub CI run [`32911228533`](https://github.com/bultodepapas/Dog-RGB/actions/runs/32911228533); no M1.11–M1.16 CI claim is made.
...
- [x] ✅ The Next.js workspace and visual shell are scaffolded with pinned dependencies.
  ...
  - Boundary: the current app completes M1.1–M1.16: Supabase/Auth boundaries, fresh server authorization, protected shell, transactional dog creation, ephemeral claim issuance, simulator pairing/upload, Today, keyset-paginated History, recording detail/points, brightness desired/reported convergence, bounded collar diagnostics/revoke, the twice-clean automated owner journey, identity/object-authorization gate, deterministic transport/fault gate, and cross-surface privacy/cache gate. M1.17–M1.18 accessibility/performance gates remain pending.
...
- [x] ✅ Product web application vertical features: recording detail, brightness desired/reported, and collar diagnostics/revoke.
  ...
  - Boundary: the unified owner journey, identity/object-denial gate, local deterministic transport/fault gate, and cross-surface privacy/cache gate are complete; M1.17–M1.18 accessibility/performance gates remain pending and must not be inferred from earlier feature, fault, or privacy evidence.
...
The text above records the original M0 recovery sequence. Its former evidence gaps were subsequently closed. The broad M1.14 local review, on 2026-08-26 with isolated Node `24.18.0`, replayed all 16 migrations from a clean reset, passed 492/492 pgTAP assertions, database lint/advisors, generated-asset/contracts/lint/type/unit/secret checks, 122/122 portal tests, and the production portal build. The M1.15 review rebuilt the production portal, then passed the owner journey twice, the exact two-owner/editor/viewer/deleted-Auth authorization matrix twice, and all ten deterministic fault outcomes twice. M1.16 added two further clean resets and passed eight ordered privacy/cache checkpoints per cycle across built/runtime/browser/cache/log/artifact surfaces with every frozen leak counter at zero. Each phase had its own migration/seed reset with zero retry where Playwright Test applies; fault cycles also proved one preserved full-stack restart and both forced sync/revoke lock orders. M0 and M1.1–M1.16 are complete on the evidence scopes stated in their ledger entries; remote CI was not run or inspected for M1.11–M1.16.
...
- [x] ✅ M1.14 Add adversarial identity and object-authorization coverage using the M1.13 harness.
  - Hard scope: freeze and attack only the current six protected route shapes, four product/object Server Actions, eleven authenticated `api` table projections, five authenticated user RPCs, and the existing user claim Edge endpoint. Reuse the real local Auth, simulator, production portal, one-worker/zero-retry runner, teardown, and secret-safe artifact controls. Add no hosted dependency, service-role browser path, generic fuzzing framework, Realtime, transport-fault behavior, or new product feature.
  - Fixture/role boundary: each clean authorization cycle creates owner A/dog A and owner B/dog B, plus editor and viewer memberships on dog A. Both collars pair and upload through the real Edge/simulator path; dog A has summary and desired/reported configuration truth. Acquire real password sessions. The final stale stage hard-deletes only the non-creator viewer after proving its access JWT is unexpired and carries `sub` plus `session_id`; do not promise immediate rejection of a merely signed-out but otherwise valid JWT.
  - Denial boundary: canonical anonymous routes redirect to login. Malformed, missing, inaccessible, cross-dog, and cross-recording portal objects share one generic 404. Missing and inaccessible RLS reads both return `200 []`; crafted claim/brightness/revoke form targets share action-specific bounded guidance; unauthorized RPCs share allowlisted authorization errors while malformed transport remains distinct. No denial may reveal object existence, SQL, identity, row, hash, or private error detail.
  - Live-Auth prerequisite: `create_dog_v1`, deletion-request replay, and deletion-job lookup must reject a null, absent, or soft-deleted Auth subject before validation, replay lookup, result access, or mutation. Preserve the deletion-request function's pinned UTC setting. Cross-identity existing and unknown deletion request IDs return the same `42501 not_authorized`; same-requester changed-body reuse retains its distinct idempotency error. Password update uses fresh Auth-server identity, and claim issuance accepts exactly one `dogId` form field.
  - Zero-effect/artifact boundary: freeze exact catalog allowlists for all eleven readable tables and five executable user RPCs. Assert the exact primary/foreign-key scope of every returned projection row. Compare a fixture-scoped canonical SHA-256 snapshot of public rows plus claims, credentials, sync receipts, chunks, configuration state, and deletion tombstones/jobs/receipts before/after denied clusters and again after the expected viewer cascade. Keep tokens, passwords, claim codes, credentials, and raw responses in memory; validate pass/failure artifacts against one exact counts/checkpoints-only schema and remove all Playwright failure context.
  - Acceptance: both clean authorization cycles must prove anonymous denial, reciprocal owner isolation, editor write/viewer read-only controls, every raw projection, every user RPC, claim Edge roles, missing/inaccessible equivalence, crafted Server Action objects, deleted-Auth page/action/REST/RPC/Edge denial, and exact zero unauthorized mutation. The complete existing owner journey must still pass independently in both cycles.
  - Explicit non-goals: session-table validation for ordinary logout, broad JWT revocation architecture, MFA, organizations, multi-dog product UX, generic penetration testing, transport restart/loss/replay injection, exhaustive cache/log/trace privacy, the full accessibility matrix, performance budgets, hosted deployment, firmware networking, or physical security proof.
  - Owner: Codex (implementation); repository owner (acceptance)
  - Target date/window: completed 2026-08-26 (America/Bogota)
  - Implementation commit/PR: local commit created with this plan update; hash recorded in Git history; not pushed
  - Evidence artifact or command: 16 clean-replayed migrations; 492/492 pgTAP assertions across 21 files; 122/122 portal tests; database lint/advisors; `npm run phase1:check`; production Next `16.3.1` build; `node tools/portal-e2e/run.mjs --clean`; M1.13 owner journey 2/2 and M1.14 authorization matrix 2/2, one worker and zero retries after four independent resets; exact 11-table/5-RPC catalog and row scope, two-owner/editor/viewer/deleted-Auth matrix, crafted form objects, deletion-aware zero-effect graph digests, exact sanitized artifact schema, and reusable final teardown; [M1.14 identity/object authorization evidence](../cloud/m114-identity-object-authorization-evidence.md); CI intentionally not run or inspected per task instruction
  - Decision/result: PASS; the local portal/Data API exposes only the frozen object surface, missing and inaccessible objects converge within each surface's bounded taxonomy, owner B cannot observe or mutate owner A, editor/viewer roles remain exact, and a deleted user's still-signed token cannot recover retained RPC results or create any effect. No transport-fault, exhaustive privacy, accessibility, performance, hosted, firmware, or physical scope was absorbed.
- [x] ✅ M1.15 Add deterministic transport/fault coverage at the existing Edge/RPC/simulator boundary.
  - Hard scope: raw device `POST` bytes through the existing local Edge endpoint, authoritative RPC/database state, a tiny simulated pending-request identity, and two forced sync/revoke schedules at the transaction seam. Reuse the M1.13/M1.14 reset/readiness/secrets/artifact/teardown owner; add no browser project, database migration, proxy, production fault hook, random latency/drop, automatic retry, hosted dependency, or firmware behavior.
  - Loss/replay boundary: commit one chunk plus one AP configuration mutation, deliberately cancel the first success body, prove the request remains pending with one exact committed receipt/effect, perform `supabase stop` without `--no-backup` plus `supabase start`, re-prove service/environment readiness and preserved state, then resend byte-identical content. Reclaim only after the response deep-equals the stored receipt and the ACK matches boot/chunk/point-count/through-sequence/hash; a same-ID valid changed body must return 409 with no `Retry-After` and zero state change.
  - Telemetry/configuration boundary: deliver disjoint ranges as separate later-then-earlier requests and require exact ACK, receipt, chunk, point-to-chunk, hash, and recording-bound rows; reject a new persisted overlap with 422 and no receipt. In one unknown-clock request, accept a gap point with null time/location/speed and `unknown` recording quality plus a `gps_quality` AP mutation rebased through `fallback_received`; exact replay must return the stored server time/HLC/outcome without advancing telemetry, revision/head, or collar-wide HLC state. Pull brightness v1, advance desired head to v2, report applied v1, and require exact v1 report and v2 head/response bodies and hashes so current desired remains pending.
  - Revocation/concurrency boundary: after owner revocation, fresh sync and even exact replay of an older committed sync both return 403 before receipt disclosure and leave the complete graph unchanged. Force sync-first and revoke-first with named PostgreSQL sessions; observe the second session through `pg_blocking_pids()` before releasing the holder. Sync-first yields one committed receipt then coherent revocation; revoke-first yields `device_revoked` and zero sync receipt. Both end with collar/credentials revoked together, zero processing receipt, and no deadlock/timeout.
  - Artifact/acceptance: compare ordered full collar, redacted credential, receipt/response, chunk, point, recording, revision/head/report, and collar-wide HLC state internally. Retain only exact fault/checkpoint enums and frozen pass counts; a caught failure must finalize and validate the same schema with `phase=failed`, the exact completed-checkpoint prefix, and zero counts before the original error is rethrown. Reject artifacts containing fixture IDs, credentials, claims, tokens, hashes, coordinates, raw bodies/responses, SQL, or errors. The M1.13 owner, M1.14 authorization, and M1.15 fault gates must each pass from two independent resets with one worker/zero retry where Playwright applies; each M1.15 cycle must include one preserved restart and both forced lock schedules.
  - Explicit non-goals: exhaustive privacy/cache scanning, accessibility, performance/load, generic chaos/fuzz infrastructure, connection-quality simulation, Realtime, device power loss/outbox reclaim implementation, firmware networking, TLS/hosted equivalence, deployment, or physical-collar proof.
  - Owner: Codex (implementation); repository owner (acceptance)
  - Target date/window: completed 2026-08-26 (America/Bogota)
  - Implementation commit/PR: local commit created with this plan update; hash recorded in Git history; not pushed
  - Evidence artifact or command: `node tools/portal-e2e/run.mjs --clean`; production Next `16.3.1` build; pinned Node `24.18.0`, Supabase CLI `2.113.0`, local PostgreSQL 17, and Playwright `1.62.1`; 16 migrations replayed before each of six independent phase cycles; M1.13 owner journey 2/2, M1.14 authorization matrix 2/2, and M1.15 ten-outcome fault matrix 2/2; two preserved full-stack restarts; four forced lock schedules; exact ACK/stored-response/full-state assertions; both artifacts retained the frozen counts of 6 receipts, 4 chunks, 7 points, 3 recordings, 3 revisions, 2 heads, 1 stale report, and 2 race schedules; final reusable reset; [M1.15 deterministic transport/fault evidence](../cloud/m115-deterministic-fault-evidence.md); CI intentionally not run or inspected per task instruction
  - Decision/result: PASS; committed-response loss plus a full preserved local restart retains one immutable pending request and returns the stored ACK on exact resend without any logical duplicate. Valid same-ID mutation, persisted overlap, stale report, revoked access, and both forced sync/revoke schedules produce the frozen response and exact zero/serial effect. No schema, browser, hosted, firmware, privacy, accessibility, or performance scope was absorbed.
- [x] ✅ M1.16 Run the cross-surface privacy and cache leak gate.
  - Hard scope: extend only the existing local production-build M1.13–M1.15 orchestrator with a focused `--m116-only` path and two fresh M1.14 two-owner/simulator fixtures. Inspect current built/runtime/browser/cache/log/artifact surfaces; add no migration, application feature, third-party scanner, hosted request, proxy, tracing product, or generic penetration framework.
  - Prohibited-value boundary: keep raw claim codes, device credentials/bearers, fixture tokens/passwords, and local service/JWT/database secrets in orchestrator memory. Reject them from built files, HTML/RSC/action responses, non-login action/URL data, console/errors, browser state, portal/Supabase logs, and retained artifacts. Reject owner B email/dog name from anonymous and owner A content, plus the exact seeded coordinate integers/renderings everywhere except the authorized owner recording detail. The login password is allowed only in its required loopback login POST body; Supabase session material is allowed only in the expected auth-cookie namespace.
  - Cache/route boundary: require protected HTML and explicit RSC to be non-public `private, no-store`; require Server Action and `Set-Cookie` responses to be non-public `no-store`. Public immutable Next assets remain cacheable only after their built bytes pass the secret scan. The bounded owner recording-detail HTML/RSC may contain its accessible three-row route table only on the exact owner dog/recording route and only with `private, no-store`; Today, History, configuration, denial, action, URL, storage, log, and artifact surfaces may not contain it.
  - Browser/log boundary: inspect anonymous home/login/protected redirect, owner Today/History/detail/configuration, cross-owner detail denial, an explicit authenticated RSC fetch, and the real brightness Server Action. Require zero unexpected Web Storage/IndexedDB/Cache Storage entries, cookie names outside Supabase auth, external/analytics requests, workers/service workers/WebSockets, or unexplained console/page errors. Consume only the one exact generic 404 caused by the deliberate cross-owner denial. Scan bounded in-memory production portal output and every running local `*_Dog-RGB-1` container log since fixture creation, including PostgreSQL and Edge runtime; make no hosted Logs Explorer claim.
  - Artifact/acceptance: keep trace/screenshot/video/HAR disabled; reject retained rich-artifact extensions; retain only an exact JSON schema containing phase, cycle, the eight ordered surface/checkpoint enums, and eight zero-finding counters. A caught failure must retain the same schema with the exact completed-checkpoint prefix before rethrow. Both independent clean cycles and the final reusable reset must pass; a source/bundle-only scan or one clean cycle cannot close the gate.
  - Explicit non-goals: accessibility/responsive acceptance, performance/load, visual redesign, hosted Vercel/Supabase/CDN/log parity, browser-extension or OS inspection, third-party analytics setup, generic secret management, firmware networking, TLS, physical hardware, or production privacy certification.
  - Owner: Codex (implementation); repository owner (acceptance)
  - Target date/window: completed 2026-08-26 (America/Bogota)
  - Implementation commit/PR: local commit created with this plan update; hash recorded in Git history; not pushed
  - Evidence artifact or command: production Next `16.3.1` build/start; checksum-verified Node `24.18.0`, Playwright `1.62.1`, Supabase CLI `2.113.0`, local PostgreSQL 17, and local Edge runtime; focused `node tools/portal-e2e/run.mjs --clean --m116-only` plus combined `node tools/portal-e2e/run.mjs --clean`; 16 migrations replayed before each independent cycle; M1.13–M1.16 each passed 2/2 in the combined eight-reset run, then tightened M1.16 passed 2/2 again; eight ordered privacy checkpoints and eight exact zero-finding counters per cycle; request-scoped HTML/RSC/action/cache/cookie/storage/network/console/worker/log/artifact assertions; final reusable reset; read-only post-run PostgreSQL 17/16-migration query; [M1.16 privacy/cache evidence](../cloud/m116-privacy-cache-evidence.md); CI intentionally not run or inspected
  - Decision/result: PASS locally; no fixture/device/infrastructure secret, unauthorized identity, internal-error signature, or misplaced route payload was found across the frozen surfaces. Protected HTML/RSC remained `private, no-store`; actions and cookie-setting responses remained non-public `no-store`; the authorized route table appeared only on owner recording detail. This closes only deterministic local M1.16 and does not imply M1.17 accessibility, M1.18 performance, hosted, firmware, or physical proof.
...
Research was refreshed on 2026-08-24 using primary/official sources and rechecked on 2026-08-26 through the M1.16 boundary. The implementation consequence is part of the plan; links must be revalidated when their phase begins.
...
| R43 | Private dynamic response caching | Current Next.js guidance distinguishes user-specific dynamic content from public cached output and documents CDN cache directives separately. | Keep protected collar HTML/RSC dynamic and non-cacheable; M1.12 verified one authenticated response and M1.16 now scans the frozen cache/static surfaces. [Caching without Cache Components](https://nextjs.org/docs/app/guides/caching-without-cache-components), [CDN caching](https://nextjs.org/docs/app/guides/cdn-caching) |
...
| R52 | Deleted users and application references | Supabase documents that Auth-user deletion does not automatically remove application rows unless the schema defines the corresponding foreign-key behavior, and that application user tables should reference `auth.users`. | M1.14 deletes only a fixture viewer whose profile/membership rows cascade, verifies those exact removals, and separately guards retained fingerprint-only deletion receipts before any RPC result is returned. [Supabase user management](https://supabase.com/docs/guides/auth/managing-user-data) |
| R53 | JWT lifetime versus fresh identity | Supabase documents that access tokens remain valid until expiry and sessions are represented by a `session_id` claim; `getUser()` performs an Auth-server validation rather than trusting local token storage alone. | M1.14 does not claim instant ordinary-logout revocation. It uses a hard-deleted-user case, requires fresh `getUser()` for protected portal/Edge mutations, and explicitly guards retained security-definer RPC results that otherwise rely only on `auth.uid()`. [Supabase sessions](https://supabase.com/docs/guides/auth/sessions), [`getUser()`](https://supabase.com/docs/reference/javascript/auth-getuser) |
| R54 | Preserved versus destructive local restart | Supabase documents that ordinary `supabase stop` preserves local Docker resources/data, while `--no-backup` deletes volumes and `db reset` destroys/recreates the database from migrations and seed. | M1.15 uses ordinary stop/start only between the lost response and exact resend; any reset, `--no-backup`, recreated fixture, or changed request identity invalidates restart evidence. [Supabase CLI reference](https://supabase.com/docs/reference/cli/supabase-start), [local workflow](https://supabase.com/docs/guides/local-development/cli-workflows) |
| R55 | Local Edge runtime boundary | Supabase documents that `supabase start` brings up the complete local stack, including Edge Functions, and that the CLI uses its Edge Runtime for local execution. | Exercise the real loopback Edge endpoint before and after restart, recheck Auth/Data API/Edge/Mailpit readiness, and make only a local-runtime claim; no mock handler or hosted parity claim closes M1.15. [Edge development environment](https://supabase.com/docs/guides/functions/development-environment) |
| R56 | Edge gateway JWT configuration | Supabase Edge Functions require JWT verification by default; `verify_jwt = false` intentionally bypasses only that platform check and requires endpoint-owned authentication. | Preserve `verify_jwt = true` for the user claim endpoint and the existing `false` setting for device claim/sync/revoke, whose bounded claim or device bearer is validated inside the handler/RPC chain; M1.15 must not alter authentication to make faults injectable. [Function configuration](https://supabase.com/docs/guides/functions/function-configuration) |
| R57 | Observable database lock ordering | PostgreSQL row locks block conflicting writers until transaction end, and `pg_blocking_pids()` exposes the sessions blocking a backend. | Force each sync/revoke order with a transaction holder plus a named waiter, require the waiter to be observably blocked before releasing the holder, and fail deadlock/timeout output. Promise scheduling or sleeps are not ordering proof. [PostgreSQL explicit locking](https://www.postgresql.org/docs/17/explicit-locking.html), [system information functions](https://www.postgresql.org/docs/17/functions-info.html) |
| R58 | Application-level test isolation | Supabase distinguishes application-level tests from rollback-isolated database tests and recommends unique fixture identities/isolated data sets because application tests cannot rely on per-test transaction rollback. | Give each clean M1.15 cycle fresh pairing/race identities, reset before the phase, compare only its collar-scoped state, and retain no dependence on the earlier owner/authorization fixture. [Supabase testing overview](https://supabase.com/docs/guides/local-development/testing/overview) |
| R59 | Retry evidence and flaky passes | Playwright documents that retries rerun failures in new workers and classify a later pass as flaky; the CLI default is zero retries. | Retain `retries: 0`; run two explicit clean phase cycles. M1.15 raw device POSTs have no hidden retry, and a permanent 403/409/422 is never reclassified as an ACK. [Playwright retries](https://playwright.dev/docs/test-retries), [CLI](https://playwright.dev/docs/test-cli) |
| R60 | Cached SSR session leakage | Supabase warns that a cached `Set-Cookie` response can deliver one user's session to another, requires one SSR client per request, and passes cache directives through current `@supabase/ssr` cookie writes. | M1.16 treats every cookie-setting response as non-public/no-store, reuses the request-scoped server client, and refuses to infer safety from RLS alone. [Supabase SSR advanced guide](https://supabase.com/docs/guides/auth/server-side/advanced-guide) |
| R61 | Dynamic versus static Next responses | Current Next.js self-hosting guidance gives dynamically rendered App Router responses private/no-cache/no-store semantics while static assets may be public immutable. | Require `private, no-store` for protected HTML/RSC, allow public static assets only after built-byte scanning, and do not globally disable useful static caching. [Next.js self-hosting](https://nextjs.org/docs/app/guides/self-hosting) |
| R62 | Sensitive CDN caching | Vercel documents `no-store` as the directive for sensitive data and distinguishes browser/CDN directives. | A Server Action or `Set-Cookie` response may omit redundant `private`, but it must contain non-public `no-store`; `public` or `s-maxage` fails. [Vercel cache-control headers](https://vercel.com/docs/caching/cache-control-headers) |
| R63 | Server Action exposure | Next.js treats Server Actions as public HTTP endpoints and warns that encrypted closure values still travel to the client. | Inspect the actual brightness action request and response, keep authorization in the action/DAL, and allow only the minimum bounded DTO; closure encryption is not accepted as a privacy proof. [Next.js data security](https://nextjs.org/docs/15/app/guides/data-security) |
| R64 | Trace/HAR retention | Playwright traces can retain DOM snapshots, console, network details, screenshots, and sources; HAR can record request/response headers, cookies, and bodies. | Keep trace/HAR/screenshot/video disabled, reject retained rich artifacts by extension, and retain only an exact zero-findings JSON manifest. [Playwright Trace Viewer](https://playwright.dev/docs/trace-viewer), [HAR replay](https://playwright.dev/docs/mock) |
| R65 | Browser-observable surfaces | Playwright exposes console, request/response, WebSocket, worker, service-worker, cookie, and non-persistent context surfaces. | Observe every current browser channel, directly read only bounded required HTML/RSC/action responses, and require zero external/analytics requests, unexpected storage, workers, or sockets. [Playwright Page](https://playwright.dev/docs/api/class-page), [BrowserContext](https://playwright.dev/docs/api/class-browsercontext) |
| R66 | Local versus hosted logs | Supabase local Edge functions emit logs through the local runtime/terminal, whereas hosted invocation/log retention is a separate operational surface. | Scan the portal plus all current local project container logs, including database and Edge, but defer hosted Logs Explorer/retention parity to M3/M5. [Supabase Edge logging](https://supabase.com/docs/guides/functions/logging) |
...
- [x] Anonymous and user B cannot read or mutate user A's dog, collar, telemetry, recording, or configuration through UI, REST, RPC, or crafted IDs.
...
- [x] ✅ Committed-response loss and exact replay create no logical duplicate or acknowledged-data loss at the reviewed local Edge/RPC/simulator boundary.
