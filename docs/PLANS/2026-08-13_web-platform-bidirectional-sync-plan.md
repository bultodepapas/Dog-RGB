...
**Status:** Active implementation contract; M0 and M1.1–M1.8 are complete on reviewed local and CI evidence, and M1.9 is next.

**Last senior review:** 2026-08-25 (America/Bogota).

**Reviewed repository commit:** `9c882d0226a6f6a625fef499d320885bc9a0e666` (`main`; GitHub CI run [`32892001603`](https://github.com/bultodepapas/Dog-RGB/actions/runs/32892001603) passed every required job).

**Current milestone:** M1C — Minimal useful portal; M1.1–M1.8 are complete and M1.9 is next.

**Next executable task:** complete only M1.9 by replacing the History placeholder with a private, server-rendered, read-only ledger of at most 20 recordings for every collar belonging to the authorized dog. Order by `started_at DESC NULLS LAST, id DESC`; fetch at most 21 rows; expose only a validated, versioned next cursor; and make the transition from known to null start times explicit so every recording remains reachable without duplication. Do not add offset pagination, total counts, client fetching, infinite scroll, detail links/content, points, summaries, charts, maps, writes, Realtime, schema changes, simulator sync, firmware work, or M1.10+ behavior.
...
| M1 | Simulator-driven local web vertical slice | **In progress — M1.9 next** | firmware Internet integration |
...
- [ ] M1.9 History lists recordings with `(started_at,id)` keyset pagination; no large offset pagination.
  ...
  - Index/schema boundary: start with the existing `recordings_collar_started_idx`; add no migration speculatively. Capture representative `EXPLAIN (ANALYZE, BUFFERS)` evidence locally. If the bounded query misses the plan's latency/scan budget, stop M1.9 and propose one measured index change with clean-reset, size, write-overhead, and rollback evidence before implementing it.
  ...
  - Target date/window: ____________________
  - Implementation commit/PR: ____________________
  - Evidence artifact or command: focused history DTO/cursor/query tests; pgTAP and raw REST owner/viewer/non-member/anonymous matrix; representative query plan; browser empty/one-page/multi-page/known-to-null/malformed-cursor states; `npm run phase1:check`; clean local gate; production build; Next.js runtime/cache checks; keyboard/responsive/accessibility review; CI: ____________________
  - Decision/result: ____________________
...
Research was refreshed on 2026-08-24 using primary/official sources and rechecked on 2026-08-25 through the M1.8 boundary. The implementation consequence is part of the plan; links must be revalidated when their phase begins.
