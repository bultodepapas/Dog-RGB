...
GitHub Actions run `32082463534` independently passed the clean `Cloud foundation (local Supabase)` job on commit `c1c524a`; it also exposed stale generated web-asset fingerprints that made the broader workflow fail before portal/firmware execution. This slice regenerates the pinned Node 24.18.0 artifacts and moves `webui:check` into `phase1:check` so the clean foundation gate catches that drift before downstream jobs. A new remote run including the eighth migration and capacity artifact is still required.

**Still open:** the clean remote rerun including the migrated-schema capacity artifact, a disposable hosted capacity/concurrency/network run, retention activation prerequisites, and the remaining operational drills. Phase 1 is not complete while these items remain.
