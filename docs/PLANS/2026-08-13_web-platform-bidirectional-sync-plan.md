...
**Status:** Active implementation contract; M0 and M1.1–M1.10 are complete on reviewed local and CI evidence, and M1.11 is next.

**Last senior review:** 2026-08-25 (America/Bogota).

**Reviewed repository commit:** `0494fb29de8c1962b63ea65fe099dee5e69cb649` (`main`; GitHub CI run [`32911228533`](https://github.com/bultodepapas/Dog-RGB/actions/runs/32911228533) passed every required job).

**Current milestone:** M1C — Minimal useful portal; M1.1–M1.10 are complete and M1.11 is next.

**Next executable task:** complete only M1.11 by replacing the protected Configuration placeholder with the brightness schema-1 resource for the same deterministic active-collar selection already used by Today. Read current desired and exact reported evidence through one fresh user/RLS path; allow only owners/editors to submit integer brightness `1..255` through the existing `api.mutate_config_resource_v1` RPC with the displayed `base_server_version`; keep viewers read-only. Distinguish cloud-saved/pending, exact applied, exact rejected, stale-form/superseded, and stale-collar truth without claiming a cloud value is physically active. Do not add another configuration resource, collar selection/management, browser Supabase access, optimistic success, Realtime, polling, simulator upload changes, firmware cloud code, or any schema/RPC change except the separately approved same-signature correction if the mandatory database prerequisite fails.

**Current blocker:** no external blocker. M1.11 has one internal go/no-go prerequisite before portal code: prove the existing mutation RPC serializes concurrent first-head writes and treats an identical canonical body as a no-op without a new version; if either proof fails, stop and approve the narrow additive correction described under M1.11. The workstation's global Node remains `24.12.0`; verification must continue to use the checksum-verified isolated Node `24.18.0` runtime required by M0.1.
...
| M1 | Simulator-driven local web vertical slice | **In progress — M1.11 next** | firmware Internet integration |
...
- [ ] M1.11 Configuration exposes brightness only.
  ...
  - RPC prerequisite/schema boundary: before portal code, add focused rollback-only database tests for concurrent first-head writes, concurrent existing-head writes, exact mutation replay, same ID/different body, and same canonical value under a new ID. Exactly one concurrent mutation may advance the head; versions remain monotonic; exact replay returns its original result; an identical value is a no-op and does not manufacture a new revision/version. If the current RPC fails any case, stop implementation and approve one additive migration that replaces only the existing function body under the same signature/grants. Do not work around a transaction race in Next.js and do not add a table, lock service, queue, or second RPC.
  - Concurrency/failure boundary: never silently retry `stale_base_server_version` against a newer value. Preserve the user's attempted brightness in the form and label it `SUPERSEDED / REFRESH REQUIRED`; explicit refresh supplies the new base. Invalid/stale Auth follows the protected boundary; viewer, non-member, inactive/cross-dog collar, malformed response, hash/version drift, RPC/database failure, and stale form expose only bounded Spanish guidance with no SQL, identity, hash, or internal row details. Do not optimistically update desired or applied state before the validated RPC result and fresh render agree.
  ...
  - Target date/window: ____________________
  - Implementation commit/PR: ____________________
  - Evidence artifact or command: focused read/action/DTO/hash/state tests; pgTAP and raw REST/RPC owner/editor/viewer/non-member matrix; stale-form and concurrent-submit proof; desired/reported simulator E2E; `npm run phase1:check`; clean local gate; production build; Next.js runtime/cache checks; keyboard/responsive/accessibility review; CI: ____________________
  - Decision/result: ____________________
