...
Migration `20260818014827_phase1_deletion_jobs.sql` now adds the local owner-authorized dog-deletion primitive. A narrow authenticated RPC serializes exact request replay, verifies current ownership, revokes device ingress, captures a coordinate-free 21-class count manifest, writes an append-only tombstone/job, marks the dog deleting, and removes memberships in one transaction. RLS and membership-backed RPCs therefore lose access immediately. A private `service_role` worker claims one ready job with `SKIP LOCKED`, deletes at most 10,000 points per transaction, checks cumulative counts, cascades the remaining graph only after raw points are gone, and emits a hashed receipt. Thirty-four pgTAP assertions cover privilege boundaries, replay, cross-user denial, exact batches, forced failure rollback/retry and cross-dog survival. The clean HTTP drill sends two simultaneous owner RPCs, proves one job/tombstone, observes outsider `403`, deletes six simulator points in three two-row commits, and retains only coordinate-free CI evidence. Cron is not scheduled and no user-facing workflow is enabled; export, reauthentication UX, account deletion and tombstone replay remain separate gates. Detailed scope is in [the Phase 1 deletion report](../cloud/phase1-deletion-drill.md).
...
**Still open:** a deliberately disposable hosted capacity/concurrency/network run; pre-deletion export and strong-confirmation UI; account deletion; reviewed hosted scheduling/monitoring and consent activation for the implemented raw-telemetry worker; retention for the other policy classes; tombstone export/replay; a managed backup/PITR restore into a distinct hosted project; and the remaining outage/rotation/quota operational drills. Phase 1 is not complete while these items remain.
