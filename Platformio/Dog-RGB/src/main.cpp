...
static void handle_status_get() {
  ...
  wifi["ap_ssid"] = g_cfg.ap_ssid;
  wifi["ap_stations"] = ap_station_count;
  wifi["sta_connected"] = wifi_sta_connected;
  wifi["mdns"] = g_cfg.mdns;

  JsonObject gps = doc["gps"].to<JsonObject>();
  gps["fix"] = has_gps_fix;
  gps["sats"] = gps_sats;
  gps["fix_quality"] = gps_fix_quality;

  JsonObject home = doc["home"].to<JsonObject>();
  home["set"] = home_set;
  home["source"] = home_source_name(home_source);
