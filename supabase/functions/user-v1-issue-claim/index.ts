...
import {
  ...
  problem, requestIdFrom, requiredEnv, serviceRpc,
...
export default {
  fetch: withSupabase({ auth: "user" }, async (req, ctx) => {
    ...
    try {
      ...
      if (userError || !userResult.user?.email_confirmed_at) {
      ...
      return Response.json({
        protocol_version: 1, request_id: requestId, server_time: new Date().toISOString(),
